...
WORKING_DIR = os.path.join(os.environ.get("HOME"), "Reasoning360")
...
def mbpp():
    ...
    def make_map_fn(split):
        def process_fn(example, idx):
            ...
            for assertion in example["test_list"] + example.get(
                "challenge_test_list", []
            ):
                test_code += assertion + "\n"
            ...
            if not succ:
                print(
                    f"Error in canonical solution for task {example['task_id']}: {err}"
                )
                return EMPTY_RETURN

            return {
                "data_source": "code-mbpp",
                "prompt": [
                    {"role": "system", "content": SYSTEM_PROMPT},
                    {"role": "user", "content": prompt},
                ],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--root_dir", default=os.path.join(WORKING_DIR, "data"))
    parser.add_argument(
        "--train_dataset_names",
        default="taco,leetcode2k",
    ...
    train_dataset_makes = [dataset_map[name] for name in dataset_names]
    test_dataset_makes = [dataset_map[name] for name in test_dataset_names]

    train_names = "-".join([make.__name__ for make in train_dataset_makes])
    test_names = "-".join([make.__name__ for make in test_dataset_makes])

    print(f"Loading train datasets: {', '.join(dataset_names)}...")
    for make in train_dataset_makes:
        train, _ = make()
        train_datasets.append(train)

    print(f"Loading test datasets: {', '.join(test_dataset_names)}...")
    for make in test_dataset_makes:
        _, test = make()
    ...
    if args.train_sample_size is not None:
        print(
            f"Sampling {args.train_sample_size} examples from training dataset (original size: {len(train_dataset)})..."
        )
        train_indices = list(range(len(train_dataset)))
        random.shuffle(train_indices)
        train_indices = train_indices[: min(args.train_sample_size, len(train_dataset))]
        train_dataset = train_dataset.select(train_indices)

    # Sample the test data if sample size is specified
    if args.test_sample_size is not None:
        print(
            f"Sampling {args.test_sample_size} examples from test dataset (original size: {len(test_dataset)})..."
        )
        test_indices = list(range(len(test_dataset)))
        random.shuffle(test_indices)
        test_indices = test_indices[: min(args.test_sample_size, len(test_dataset))]
    ...
    # Modify output directory name based on train sample size if specified
    dir_name = f"codegen-{round(len(train_dataset) / 1000)}k-{train_names}"
